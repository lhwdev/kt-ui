...
class WidgetFunctionBodyTransformer : IrElementTransformerVoidScoped<WidgetFunctionBodyTransformer.MyScope<out IrElement>>(), UiIrPhase {
	...
	private val widgetScopeOrNull: MyScope.Widget?
		get() {
			...
			for(i in stack.lastIndex downTo 0) {
				...
				if(element.isInline) continue // TODO: BUG: in case of lambda, crossinline fun should not be passed
				else return null
			}
			return null
		}
	...
	private fun IrFunction.irWidgetDeclaration(scope: MyScope.Widget): IrStatement {
		...
		body = irBlockBody {
			...
			+irCall(if(hasReturn) UiLibraryDescriptors.BuildScope.startExpr else UiLibraryDescriptors.BuildScope.start, dispatchReceiver = irGet(buildScope), valueArguments = listOf(irInt(getLocationId(info.function))))
			...
			info.realParameters.forEachIndexed { i, parameter ->
				...
				if(defaultValue == null) {
					...
					val actualValue = irVariable(name = parameter.name.asString(), type = if(needsNullCheck) type.withHasQuestionMark(false) else type)
				...
				kind.isSkippable -> {
					+irIfThen( // skippable widgets so far do not have return value
						...
						condition = dirtyList.map { dirty ->
							irNotEquals(irAnd(irXor(irGet(dirty), irInt(sSameOnlyFlags)), irInt(sSameOnlyFlags)), irInt(0))
			...
			when {
				...
				hasReturn -> +irCall(UiLibraryDescriptors.BuildScope.endExpr, dispatchReceiver = irGet(buildScope))
				else -> +irCall(UiLibraryDescriptors.BuildScope.end, dispatchReceiver = irGet(buildScope))
			}
	...
	override fun visitWhen(expression: IrWhen): IrExpression = with(expression) {
		...
		if(conditionHasWidgetCalls || resultHasWidgetCalls) irBlock(type = type) {
			...
			if(conditionHasWidgetCalls) +irCall(
				UiLibraryDescriptors.BuildScope.start,
			...
			val value = irWhen(type = type, origin = expression.origin) {
				branches.forEachIndexed { index, branch ->
					branch.condition.transformChildren() then irBlock {
						if(conditionHasWidgetCalls)
							+irCall(UiLibraryDescriptors.BuildScope.end,
								dispatchReceiver = irGet(scope.info.buildScopeParameter))
						+irCall(UiLibraryDescriptors.BuildScope.startReplaceableGroup,
							dispatchReceiver = irGet(scope.info.buildScopeParameter),
				...
				if(branches.none { it.isElse }) orElse(irBlock {
					if(conditionHasWidgetCalls) +irCall(UiLibraryDescriptors.BuildScope.end,
						dispatchReceiver = irGet(scope.info.buildScopeParameter))
			...
			val end = irCall(UiLibraryDescriptors.BuildScope.endReplaceableGroup,
				dispatchReceiver = irGet(scope.info.buildScopeParameter))
	...
	private fun MyScope.Widget.transformLoop(loop: IrExpression) = irBlock(loop.type) {
		+irCall(UiLibraryDescriptors.BuildScope.start,
			dispatchReceiver = irGet(info.buildScopeParameter),
			valueArguments = listOf(irInt(getLocationId(loop))))
		val end = irCall(UiLibraryDescriptors.BuildScope.end,
			dispatchReceiver = irGet(info.buildScopeParameter))
		if(loop.type.isUnit()) {
			+loop
			+end
	...
	override fun visitBreakContinue(jump: IrBreakContinue) = widgetScopeOrNull?.let {
		...
		if(jump.loop != nearestLoop.irElement) {// long break/continue
			val loops = scopeStack.asReversed()
				.filterUntil { scope -> !scope.irElement.let { it is IrFunction && !it.isInline /* TODO */ } /* do not pass the function boundary */ }
				.filterIsInstance<MyScope.Loop>()
			val loopsToExtraJump = loops.indexOfFirst { it.irElement == jump.loop }
			irBlock {
				for(i in 0 until loopsToExtraJump) +irCall(UiLibraryDescriptors.BuildScope.end,
