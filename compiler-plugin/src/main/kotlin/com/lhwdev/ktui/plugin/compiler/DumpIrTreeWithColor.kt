...
private fun renderTypeAnnotations(annotations: List<IrConstructorCall>) = when {
	annotations.isEmpty() -> ""
	annotations.singleOrNull()?.symbol?.descriptor?.constructedClass?.fqNameSafe == KOTLIN_PACKAGE_FQN.child(Name.identifier("ParameterName")) ->
		(annotations.single().getValueArgument(0) as IrConst<*>).value.toString().let { "$it: " }
	else -> annotations.joinToString(prefix = "", postfix = " ", separator = " ") { "$sDimmed@[$sReset${renderAsAnnotation(it)}$sDimmed]$sReset" }
}
...
private class RenderIrElementVisitor(private val normalizeNames: Boolean = false) : IrElementVisitor<String, Nothing?> {
	...
	private fun IrSymbol.renderReference() =
		sReference +
			(if(isBound)
				owner.accept(symbolReferenceRenderer, null).replace(ConsoleColors.RESET, ConsoleColors.RESET + sReferenceColor)
			else
	...
	override fun visitSimpleFunction(declaration: IrSimpleFunction, data: Nothing?): String =
		declaration.runTrimEnd {
			"$sIdentifierDimmed$visibility ${modality.toString().toLowerCase(Locale.getDefault())}$sReset ${sIdentifier}fun$sReset ${renderOriginIfNonTrivial()}$sName$name$sReset" +
...
private fun escapeStringCharacters(s: String): String {
	...
	escapeStringCharacters(length = s.length, str = s, additionalChars = "\"", escapeSlash = true, escapeUnicode = true, buffer = buffer)
	return buffer.toString()
}

private fun escapeStringCharacters(length: Int,
								   str: String,
