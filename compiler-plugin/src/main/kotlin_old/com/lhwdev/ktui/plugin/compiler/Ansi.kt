...
object Ansi {
	...
	// high intensity
	val brightBlack = AnsiColor.black.background
	val brightRed = AnsiColor.red.background
	val brightGreen = AnsiColor.green.background
	val brightYellow = AnsiColor.yellow.background
	val brightBlue = AnsiColor.blue.background
	val brightPurple = AnsiColor.purple.background
	val brightCyan = AnsiColor.cyan.background
	val brightWhite = AnsiColor.white.background
}
...
sealed class AnsiItem {
	internal abstract fun build(): String
	override fun toString() = "\u001b[${build()}m"
}

class AnsiItemList(val items: List<AnsiItem>) : AnsiItem() {
	override fun build() = items.joinToString(separator = ";") { it.build() }
}
