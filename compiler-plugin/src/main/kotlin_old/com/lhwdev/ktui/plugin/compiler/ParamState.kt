...
fun IrElementScope.stateOf(
	target: IrExpression,
	parameters: List<IrValueParameterSymbol>,
	dirtyList: List<IrValueSymbol>,
	shifts: Int
): IrExpression? {
...
class ParamStateList(val variables: List<IrValueParameterSymbol>) {
	...
	fun IrElementScope.shiftBitAt(index: Int, toIndex: Int): IrExpression {
		...
		return irAnd(
			irShr(irGet(variables[index / PARAM_COUNT_PER_INT]), irInt(deltaIndex % PARAM_COUNT_PER_INT * 2)),
			irInt(0b11 shl toIndex)
		)
	}
