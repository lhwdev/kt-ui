...
private val sStatelessKotlinClasses =
	arrayOf("Number", "Byte", "Short", "Int", "Long", "Float", "Double", "Boolean", "Char", "String")
			...
			when(this) {
				is IrVariable -> when {
					...
					(initializer as? IrCall)?.let { // remember { .. } (without key) is always persistent during the lifecycle
						it.symbol.descriptor.fqNameSafe == UiLibraryNames.REMEMBER &&
				...
				is IrValueParameter -> when { // @Key param: Type is always persistent
					annotations.any { it.symbol.ownerOrNull == UiLibrary.key } -> DependType.static
