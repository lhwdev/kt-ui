...
fun IrElement.dumpSrcColored(context: IrPluginContext, config: SourcePrintConfig = SourcePrintConfig.default) =
	buildString {
//		patchDeclarationParentsChildren()
		acceptVoid(IrAllSourcePrinterVisitor(this, context, config))
	}

fun IrElement.logSrcColored(context: IrPluginContext, config: SourcePrintConfig = SourcePrintConfig.default) {
//	patchDeclarationParentsChildren()
	fixIndents {
...
fun IrElement.dumpSrcHeadColored(context: IrPluginContext, config: SourcePrintConfig = SourcePrintConfig.default) =
	buildString {
//		patchDeclarationParentsChildren()
		acceptVoid(IrDeclarationHeadPrinterVisitor(this, context, config))
	}

fun IrElement.logSrcHeadColored(context: IrPluginContext, config: SourcePrintConfig = SourcePrintConfig.default) {
//	patchDeclarationParentsChildren()
	fixIndents {
...
fun IrType.dumpColored(context: IrPluginContext, config: SourcePrintConfig = SourcePrintConfig.default) = buildString {
...
//	}
	...
	inline fun <T> Iterable<T>.printJoin(
							...
							try {
								...
								if(delegatedMember != null) {
									...
									val toGetDelegate = when(delegatedMember) {
										is IrProperty -> {
											val getterCall =
												delegatedMember.getter!!.body!!.asExpression() as IrCall /* getter <get-???> */
...
//							delegatingConstructorCall?.printCallTypeArguments() // already printed in type
				...
				debug {
					if(superTypes.count { it.classOrNull?.descriptor?.kind != ClassKind.INTERFACE } > 1) {
						...
						parenGroup {
							superTypes.filter { it.classOrNull?.descriptor?.kind != ClassKind.INTERFACE }
								.printJoin(",") {
									it.print()
								}
						}
	...
	override fun visitFunctionNew(declaration: IrFunction): Unit = declare(declaration) {
		...
		debug {
			...
			val inconsistentValueParameters =
				valueParameters.withIndex().filterIndexed { i, parameter -> parameter.value.index != i }
			...
			val inconsistentTypeParameters =
				typeParameters.withIndex().filterIndexed { i, parameter -> parameter.value.index != i }
	...
	override fun visitVariable(declaration: IrVariable) = declare(declaration) {
		...
		if(origin == IrDeclarationOrigin.IR_TEMPORARY_VARIABLE) stylePrefix(Ansi.italic) {
			print(
				name,
				Type.localVariable
			)
...
//		printCallPrefix()
	...
	fun IrFunctionAccessExpression.printCallValueArguments(
		prefix: CharSequence = "(",
		postfix: CharSequence = ")",
...
//		val superQualifiedSymbol = superQualifierSymbol
	...
	override fun visitStringConcatenation(expression: IrStringConcatenation) = expr(expression) {
		...
		arguments.forEach { item ->
			compact()
			when {
				item is IrConst<*> && item.kind == IrConstKind.String -> print(
					item.value.toString(),
					Type.stringLiteral
				)
				item is IrGetValue -> {
