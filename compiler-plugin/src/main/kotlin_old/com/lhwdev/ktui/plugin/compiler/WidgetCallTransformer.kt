...
class WidgetCallTransformer : IrWidgetElementTransformerVoidScoped(), UiIrPhase {
	...
	override fun visitFunctionAccess(
		...
		val isWidgetLambdaInvocation =
			dispatchReceiver?.type?.isWidget() ?: false && symbol.descriptor.isFunctionInvoke()
		...
		when {
			...
			isWidgetLambdaInvocation -> {
				...
				val functionClass =
					irBuiltIns.function(allCount + if(symbol.descriptor.extensionReceiverParameter == null) 0 else 1)
				val newInvocation = functionClass.owner.functions.first { it.name.asString() == "invoke" }
	...
	private fun IrFunctionAccessExpression.irWidgetCall(info: WidgetTransformationInfoForCall): IrExpression {
		...
		return irCall(
			...
			info.changedParameters.forEachIndexed { changedIndex, changedParam ->
				...
				for(realIndex in changedIndex * PARAM_COUNT_PER_INT until minOf(
					realParameters.size,
					(changedIndex + 1) * PARAM_COUNT_PER_INT
				)) {
					...
					if(argument == null) {
						...
						putValueArgument(
							realIndex,
							if(realParam.descriptor.isVararg) irNull() else realParam.descriptor.type.toIrType()
								.defaultValue()
						)
					} else {
